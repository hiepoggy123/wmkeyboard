...
- **Panel tools** `uncommon` — 12 tools in the Panels group
  ...
  - Vocabulary tool — GRE-level word cards, nudges and flashcards from downloadable vocabulary packs (`vocab/<lang>/<id>.wmvocab.json.gz` in wmkeyboard-data, built from the wordcheck lists with Wiktionary via kaikki.org, WordNet, CMUdict and wordfreq)
    ...
    - App screens — packs screen cached per pack-state (`InstalledPacksCache`) with translations folded under a summary row; browse filters scroll sideways; the add-word sheet is full-height, scrolling, sectioned, Save in the header; bulk add keeps its text as a `TextFieldValue` (hoisted String doubled words); the word card is sectioned (hero, Meaning grouped by POS, Related, Remember it, In your languages with romanisation-first for `_rom` layouts, Where it comes from folded, More folded)
