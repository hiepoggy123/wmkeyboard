...
- **[Getting started](GETTING_STARTED.md)** — build a working cipher tool from an
  empty folder. Assumes you have never written Lua.
- **[API reference](API_REFERENCE.md)** — the manifest, every widget, every
  event, every `wm.*` function, and every limit.
- **[Permissions](PERMISSIONS.md)** — the one thing a plugin can ask for, and the
  many it cannot.
- **[Security](SECURITY.md)** — what the sandbox does, what it deliberately does
...
them — the sandbox has no function for any of it. See [SECURITY.md](SECURITY.md)
