...
- **Assets stay where the manifest says they are.** A repository can't point an asset at a different host than its own manifest, and can't walk outside its own folder with a `../` path. One manifest can't quietly serve files it doesn't own.
