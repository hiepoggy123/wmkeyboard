...
- **Tapping Refresh**, the button next to Add repository on the Addons screen, refreshes all repositories at once. A single repository's overflow menu (⋮) has its own **Refresh** that fetches just that one. Refresh always fetches whatever those two settings say, because pressing it is the request.
...
**The manifest changes remotely.** If a repository drops an addon, goes offline, or the publisher renames something, the catalogue and detail pages fall back to a page built entirely from what was saved at install time: name, type, version, description, repository. It says plainly that without a manifest to compare to, there's nothing to check for updates against. Uninstalling still works from this fallback page. Only the version comparison is unavailable.
...
|---|---|---|
| Theme | Removed from your custom themes, falling back to the default theme if it was active | Background images copied into app storage when the theme was installed are not deleted (a harmless leftover file) |
...
| Font | Removed from your installed fonts | None |
...
<SettingsPath path="Add-ons" />

The Addons screen itself holds the only addon-related settings. Nothing addon-specific lives in the main settings store, since a repository list and an installed-addons record are files rather than preferences. From here:
...
- **Refresh** re-fetches every added repository's manifest, whatever those two settings say.
...
- **Downloads resume.** If an update or install is interrupted partway through, retrying continues from where it left off instead of starting over. It's the same range-request-and-partial-file approach the on-device AI model downloads use elsewhere in the app. [Downloadable wordlists](/languages/dictionaries/) are the exception: a dictionary download restarts from the top, since it's a capped stream that's deliberately cut off early and can't be resumed midway.
