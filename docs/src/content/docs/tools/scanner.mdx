...
- **Text recognition**: **Automatic** (the default), **ML Kit** or **Tesseract**. See [which languages it reads](#which-languages-it-reads). ML Kit ignores the text's language, so the viewfinder shows no language chip with it. Tesseract reads Latin script too, usually slower than ML Kit.

<SettingsPath path="Tools / Text scan (OCR) / Tesseract languages" />
