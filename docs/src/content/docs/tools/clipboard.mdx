...
Detection is conservative on purpose. A 4-digit year is never mistaken for a code, and a run of digits after a currency symbol or a `#` is never mistaken for a phone number. Google's `G-123456` verification prefix is stripped, so only the code itself pastes. Pulling a fragment out of a longer message is the panel's job alone. The suggestion strip only ever offers a copied code when the clip *is* the bare code and the field you are in asks for digits, which is the **Suggest copied codes in code boxes** rule under [Options](#options).
