...
<PhoneFrame caption="The text-editing panel: a d-pad for moving the cursor, Select/Select all/Copy/Paste on the right, and Home/End/Backspace along the bottom.">
  <Shot id="tools/text-edit-panel" alt="The text-editing panel: a d-pad for moving the cursor, Select/Select all/Copy/Paste on the right, and Home/End/Backspace along the bottom." />
</PhoneFrame>
...
Once something is selected, the keyboard can offer what to do with it: [Selection actions](/smart/selection-actions/) puts the edits, a find and replace, the case options, Call, Open, QR and a good deal more above the keys, depending on what you selected. That bar is off by default and lives under Advanced.

Holding <KeyCap>◀</KeyCap>, <KeyCap>▶</KeyCap>, <KeyCap>▲</KeyCap>, <KeyCap>▼</KeyCap>, or <KeyCap>Backspace</KeyCap> auto-repeats after the keyboard's own key repeat delay, then keeps repeating at an interval you can tune (see [Options](#options) below). That start delay defaults to 300 ms and is adjustable from 150 to 800 ms under Key press.
...
- **Select all** selects the whole field and doesn't touch selection mode either way, so a follow-up arrow key moves the caret unless selection mode was already on for some other reason. It's also a toolbar tool of its own (see [below](#one-tap-cursor-tools-for-the-toolbar)), so it needn't cost a panel.
...
A held or locked Shift reaches every cursor move you make from the keyboard itself, not only the toolbar's tools: the [spacebar cursor swipe](/typing/gestures/) on both axes, the [volume keys](/typing/gestures/) when they're set to move the cursor, and a layout's own arrow keys. Lock Shift and scrub the spacebar and you're dragging a selection out with one thumb each. The text-editing panel as shipped has no Shift key, so its own <KeyCap>Select</KeyCap> toggle is the answer there, unless you put a <KeyCap>Shift</KeyCap> key on the panel in its [layout editor](/languages/custom-layouts/#panel-layouts), where it works exactly like this.
...
<PhoneFrame caption="The numpad panel: a 4×4 grid in phone or calculator digit order, with Backspace repeating on hold.">
  <Shot id="tools/numpad-panel" alt="The numpad panel: a 4×4 grid in phone or calculator digit order, with Backspace repeating on hold." />
</PhoneFrame>
...
**Key repeat interval**: default **60 ms**, adjustable from 30–200 ms. It sets the pause between repeats while you hold an arrow key in the panel, and while you hold one of the toolbar's cursor tools. The panel's two delete keys are the exception: they repeat at the keyboard's own **Key repeat interval for delete** under Key press, because they *are* the delete keys — see below. Lower is faster. The delay before repeating starts is the keyboard's own key repeat delay in the panel, and the system's long-press timeout on the toolbar.
...
Every key on the panel is a key whose action is a **text editing action**, one of twenty-one: the four arrows, word left and right, page up and down, start and end of the line, start and end of the text, Select, Select all, Select word, Select line, Cut, Copy, Paste, Delete and Forward delete. Pick a key in the preview and the sheet lets you change its action, its width in grid units, how many rows it covers, and its alternates for press and hold. You can also put ordinary keys on the pad: Enter, a tool key, or anything else the Action picker offers.

**Press and hold only works on keys that don't repeat.** The four arrows, word left and right, page up and down, Delete and Forward delete all repeat while you hold them, at the repeat interval above, and that's the point of holding them, so the sheet doesn't offer alternates on those. Everything else (Home, End, start and end of the text, Select, the selects, Cut, Copy, Paste) has a hold going spare, and that's where an alternate goes. Holding such a key opens the alternates popup, the way it does on any key of the keyboard, and sliding onto the entry and letting go runs it.

**The pad's Delete and Forward delete are the keyboard's own delete keys.** They carry every gesture the key on the letters row has: a swipe takes words or characters with the same select-first preview, **Holding delete clears words** applies, and each stops at the end of the text it eats into. They use the delete key's repeat interval rather than the panel's for the same reason. Delete takes what is behind the cursor; Forward delete takes what is ahead of it, and its swipe runs rightward. See [Gestures](/typing/gestures/#deleting-more-than-a-character-with-backspace) for the settings that shape both.
...
- **Two Typing-screen toggles change what Select mode does elsewhere on the keyboard**, even though neither is on this page's own settings screens. Typing a bracket or quote with text selected wraps the selection in the pair instead of replacing it (on by default). Either half works: `(` and `)` both turn a selected "abc" into "(abc)". Tapping Shift with text selected cycles its case instead of arming Shift for the next letter (also on by default, and a no-op in caseless scripts like Bengali). Both live under <SettingsPath path="Typing / Automatic corrections" />.
