...
- Every row also shows a live "Inserts as: …" preview whenever the expanded text would differ from what you typed, and "Auto-expands from: …" if you've set a trigger (or the pattern itself, if you set one of those).
...
Beyond that, there's nothing global to configure about expansion: no on/off switch separate from the tool. How matching works is set per snippet instead, in the add/edit dialog — word or pattern, and how far back a pattern may reach. Having no pattern snippets at all is what keeps the matching machinery out of the typing path entirely.
