...
One snippet can answer to several triggers. The **Other triggers** field in the add/edit dialog takes a comma-separated list, and each one fires the same snippet in exactly the same way, punctuation prefixes included. Sixteen is the limit, and a spelling that repeats the main trigger or an earlier alias is quietly dropped.
