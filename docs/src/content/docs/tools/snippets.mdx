...
- The trash icon at the end of the row **deletes** it immediately: no confirmation prompt, no undo. That's where nearly every accidental deletion comes from, so aim before you tap, and keep an [export](#import-and-export) around if a snippet took you a while to write.
