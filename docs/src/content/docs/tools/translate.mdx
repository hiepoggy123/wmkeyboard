...
:::note
Hardware-keyboard focus-ring navigation reaches the whole panel: <KeyCap>Tab</KeyCap> and arrow-key cycling take in the target-language chip, the picker's rows while it's open, and both action buttons, the same way they do in the GIF or search panels. One rough edge is left. <KeyCap>Esc</KeyCap> while the language picker is open closes the whole panel rather than just the picker, so press <KeyCap>Enter</KeyCap> on the chip to bring the list back.
:::
