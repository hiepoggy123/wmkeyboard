...
Open **Calendar** from the toolbar (or the toolbox if you haven't pinned it). The `<` and `>` arrows step a month at a time; a **Today** chip appears next to the month label whenever you've navigated away from the current month, and tapping it jumps straight back and reselects today. The weekday row along the top tints Friday in the accent color rather than Saturday or Sunday, since Friday is the weekend day in Bangladesh.
...
- **First calendar** — default **Bengali**. Choose from Bengali, Hijri, Chinese, Hebrew, Hindu (Saka), Persian (Solar Hijri), Buddhist (Thai solar), Japanese (era years), or None. Its day number is the one that also shows up inside each day cell, unless it's Buddhist or Japanese (whose days line up one-to-one with Gregorian, so there's nothing extra to show), in which case the cell falls back to Second calendar's day number if that one has its own.
- **Second calendar** — default **Hijri**. Same nine choices; shown in the month header and under the selected day, alongside First calendar.
...
The same First calendar / Second calendar pickers also appear on the first-run setup page during onboarding, if you've kept Calendar enabled: anything you set there is the same setting, just asked up front.

<PhoneFrame caption="Calendar options: First and second calendar pickers, and the Hijri day adjustment slider.">
  ![Calendar options: First and second calendar pickers, and the Hijri day adjustment slider.](@assets/screens/tools/calendar-options.webp)
</PhoneFrame>
...
- Calendar ships enabled by default, but it isn't one of the tools pinned to the toolbar out of the box, and it's also left out of the "recommended tools" set that onboarding offers to turn on for you. If you use the recommended-tools shortcut during setup, add Calendar back yourself under Settings → Tools.
