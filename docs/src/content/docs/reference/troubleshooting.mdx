...
- **Suggestions**, on by default. This is the master switch for the whole strip. Turning it off also stops correction candidates and next-word prediction, though [autocorrect](/smart/autocorrect/) itself keeps working. See [Suggestions & prediction](/smart/suggestions/) for the rest of that screen.
...
Media controls use a different mechanism: notification-listener access rather than a runtime permission. Grant it from **Settings → Apps → Special app access → Notification access**, where WM Keyboard is listed as **"WM Keyboard media controls."** The same rule applies afterward. Revoke or re-grant it there, and [Media controls](/tools/media/) picks up the change the next time the keyboard comes to the foreground.
