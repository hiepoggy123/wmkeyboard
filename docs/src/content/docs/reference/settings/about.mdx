...
The only real switch on this page is **Include the system log**, off by default — everything else (Version, Dictionaries) is a display-only row, and the licence and diagnostics screens are read-only browsers. See [Privacy policy](/privacy/overview/) and [Network policy](/privacy/network/) for what the app does and doesn't send over the network in general; Diagnostics itself never touches the network on its own — sharing or copying the report is something you trigger and send yourself.
