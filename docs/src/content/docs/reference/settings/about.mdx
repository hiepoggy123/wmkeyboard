...
<PhoneFrame caption="The About screen: Share, App, Updates, Feedback, Documentation, Third party, and Word lists groups.">
  <Shot id="reference/settings/about-screen" alt="The About screen: Share, App, Documentation, Third party, and Word lists groups." />
</PhoneFrame>
...
- **Short app name** is a switch, off by default. Turn it on and your app drawer and home screen call the app **WMK** instead of **WM Keyboard**, so a narrow launcher grid doesn't cut the name short. See [Changing the name under the icon](#changing-the-name-under-the-icon).
- **App language** picks the language the app's menus and settings are shown in, separate from your phone's. On the English-only download from GitHub it says so instead, and offers to fetch the version with every language. See [Changing the app's language](#changing-the-apps-language).
...
**App language** opens a list of every language the app is translated into, each written in its own script, with its name in the current language underneath. **System default**, at the top, follows your phone and names the language that means right now. Pick one and the screen restarts in it at once. Resetting the row goes back to System default.

This only changes the words the app shows you. The languages you type in are a separate list, under [Languages](/languages/overview/).

- **Android 13 and newer** already have this setting, under Settings, Apps, WM Keyboard, Language. The row here and that screen change the same value, so a change in one shows in the other. If you set a language on Android 12 and then upgrade the phone, the app moves your choice over the first time it opens.
- **Android 12 and older** have no such setting, so the app keeps the choice itself and applies it to the settings app. The keyboard stays in your phone's language on these versions.
- **Play Store builds** download a language the first time you pick it if the Play Store did not install it with the app, so the screen can take a moment to restart. If the download fails, the app shows English until the next try.
- **F-Droid builds** carry every translation.

### Getting every language on the English-only download

GitHub releases come in two kinds: `intl`, with every translation, and `en`, English only and a good deal smaller. On an `en` install the row reads **This download of the app has English only** and names how many languages the other one adds. Press it to switch:

1. A dialog says what happens next. Press **Download**.
2. The row finds the `intl` build of the newest release your install can have. That is usually the same version you already run, or a newer one if an update is out. Then it downloads the file for your phone. On a metered connection you're asked first, the same as for any update.
3. The row changes to **Install every language**. Press it when you're ready. The app and the keyboard restart once, the same as after an update.

It's the same app, signed with the same key and at the same version, so it installs over the English-only one and keeps every setting, word and theme. The download goes through the same checks as an update: the checksum from the release, then the signing key. See [What a GitHub build checks before it installs](#what-a-github-build-checks-before-it-installs).

While the switch is under way the Updates group below hides its status row, because the App language row is showing that download. **Keep English only**, under the row, stops the switch and deletes what it downloaded. A switch that fails and is left alone gives way to the ordinary update check the next time it runs. After the install, the home screen says **Every language is here** once, and App language becomes the ordinary picker.

A copy of the `en` build with no GitHub updater in it, such as one you built yourself, links to the releases page instead.

The setup wizard asks the same question as its very first page when your phone is somewhere English isn't the first language. See [Running the setup wizard](/start/setup/#running-the-setup-wizard).
...
### What a GitHub build checks before it installs
...
1. The file name has to match the release and your phone: the right edition, the same languages build as yours (every language, or English only), and the right CPU, falling back to the universal build if your CPU has no build of its own.
2. The downloaded bytes are hashed and compared against the checksum published in the release. A file that doesn't match is deleted, not installed. A release that publishes no checksum at all is refused rather than trusted.
3. The APK's signing key is compared against the key your current install is signed with. A mismatch stops there and tells you to fetch the file yourself, which is also what happens if you're running a build you compiled locally.
4. Android is asked to install it, and shows its own confirmation.

The download goes to the app's own storage and shows up on the [storage screen](#seeing-what-the-app-is-using) as **Downloaded update**, so you can delete it. Deleting it costs you the download and nothing else.
...
<PhoneFrame side caption="The open-source licences list, grouped by bundled components, downloadable data packs, and online services.">
  <Shot id="reference/settings/licenses-screen" alt="The open-source licences list, grouped by bundled components, downloadable data packs, and online services." />
</PhoneFrame>
...
**Data packs you can download** covers the frequency wordlists, offensive word lists, emoji keyword dictionaries and Noto Animated Emoji served from the WM Keyboard data repository, plus the CJK-specific conversion data: CC-CEDICT, mozc, rime-cantonese & CC-Canto, a Chinese stroke code table, and the Unicode Unihan database. Ten rows in all, since the AOSP dictionaries have their own. A <Flavor edition="full" /> build adds two more for downloads only it can make: the sticker editor's own U²-Net-P background remover (Apache-2.0) and the on-device AI models (Apache-2.0, or Google's Gemma Terms of Use for Gemma 3). A caption underneath notes these keep their upstream licences whether or not they're actually installed on your device.

**Online services** lists what each network tool talks to and under what terms: Brave Search, KLIPY & GIPHY, Wikipedia, Google Translate, Open-Meteo, Frankfurter & ExchangeRate-API, Coinbase & the other coin-price sources, the Free Dictionary API, Wiktionary & kaikki.org, Datamuse, Hugging Face, and the bring-your-own-key AI providers (Anthropic, OpenAI, Google AI, xAI, DeepSeek, and whichever OpenAI-compatible service you point the escape hatch at). None of their code or data ships inside the app; see [Network policy](/privacy/network/) for what each one actually sends and when.
...
<PhoneFrame side caption="Diagnostics: report actions, the app log with its filter and level chips, and Clear app log at the bottom.">
  <Shot id="reference/settings/diagnostics-screen" alt="Diagnostics: report actions, the app log with its filter and level chips, and Clear app log at the bottom." />
</PhoneFrame>
...
There are only four switches and one picker in this whole tree. Everything else is either a display-only row (Version, Dictionaries) or a read-only browser (the licence and diagnostics screens).

<SettingsPath path="About" />

| Setting | Control | Default |
|---|---|---|
| Short app name | Toggle | **Off** |
| App language | Choice | **System default** (on the English-only build, an offer to switch) |
...
See [Privacy at a glance](/privacy/overview/) and [Network policy](/privacy/network/) for what the app does and doesn't send over the network in general. Diagnostics itself never touches the network on its own, since sharing or copying the report is something you trigger and send yourself.
