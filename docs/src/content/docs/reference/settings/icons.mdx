...
- **Deleting the active pack** turns off "active pack" entirely, not just removing that row — any slot that was relying on it (and wasn't separately overridden) falls back to the built-in icon.
