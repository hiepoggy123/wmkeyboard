...
<SettingsPath path="Clipboard / Phone number formats" />
...
The list is not empty to begin with. It starts with one mask for your own country, read off your phone's region the same way the calendar tool picks its alternate calendars: a phone in Bangladesh opens on `+880 XXXX-XXXXXX`, one in the US on `+1 XXX-XXX-XXXX`. The table covers about 110 regions, and it's mobile numbers, so a country whose landlines run to a different length will drop those. Edit the mask, add a second one, or delete it and go back to catching every number. A region the table doesn't know, and one it deliberately leaves out because its numbers have no single length (Germany and Austria), start with nothing.

The row that opens it sits on the Clipboard tool's own settings and only appears while **Detect codes, numbers and links** is on, since there would be nothing to filter otherwise. Its subtitle counts your saved masks, or reads "Keep only the numbers that look like the ones you copy. Every number becomes a chip now" once you have deleted them all. Settings search indexes the screen either way, so searching "phone number formats" reaches it even with the toggle off.

<PhoneFrame caption="Phone number formats: the explanation, Add a format, the saved masks, and the Test a number field.">
  <Shot id="reference/settings/phone-formats-screen" alt="Phone number formats: the explanation, Add a format, the saved masks, and the Test a number field." />
</PhoneFrame>
...
A dial code written as wildcards names no country, so a mask like `+XX 1XXX-XXXXXX` can't hold you to one. Rather than reject it, the app keeps the part that still means something. It drops the wildcards where the code was, and what's left becomes a national mask. That behaves exactly as though you'd written `1XXX-XXXXXX`: a real filter for ten digits starting with a 1, from any country, and probably not the one you thought you were writing. The row still reads back the way you typed it, `+` and all, so the plus sign in it is doing nothing.
...
|---|---|---|
| **Add a format** | Opens the dialog described above | n/a |
| The saved list | One row per mask, sorted alphabetically, each with a delete icon. Empty, a caption reads "There is no format. The keyboard keeps every number." | One mask for your phone's region |
