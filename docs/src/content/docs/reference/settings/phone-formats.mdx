...
The row that opens it sits on the Clipboard tool's own settings and only appears while **Detect codes, numbers and links** is on, since there would be nothing to filter otherwise. Its subtitle counts your saved masks, or reads "Keep only the numbers that look like the ones you copy. Every number becomes a chip now" while you have none. Settings search indexes the screen either way, so searching "phone number formats" reaches it even with the toggle off.
...
| The saved list | One row per mask, sorted alphabetically, each with a delete icon. Empty, a caption reads "There is no format. The keyboard keeps every number." | Empty |
