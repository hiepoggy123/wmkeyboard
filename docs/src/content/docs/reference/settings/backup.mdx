...
- **The automatic backup's only condition is charging.** The scheduled job asks the system for a charger and nothing else: not Wi-Fi, not an unmetered connection, not an idle device. An interval is a period, not an alarm clock, so a run lands somewhere inside the window rather than on the hour. **Back up now** ignores all of it and runs immediately.
