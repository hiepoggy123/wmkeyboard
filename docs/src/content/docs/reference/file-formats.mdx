...
<PhoneFrame caption="Opening a WM Keyboard file from a file manager: one confirm dialog, then the import.">
  <Shot id="reference/import-file-confirm-dialog" alt="Opening a WM Keyboard file from a file manager: one confirm dialog, then the import." />
</PhoneFrame>
...
| `.yml` / `.yaml` | An Espanso match file | **Text expansion** → export → Espanso match file |
| `.zip` | An Espanso package | **Text expansion** → export → Espanso package |
| `.wmstickers` | A sticker pack (ZIP) | Stickers screen → export |
| `.wmicons` | An icon pack (ZIP) | Icons screen → export |
| `.wmsoundpack` | A key-sound pack: many recordings, played one after another (ZIP) | Not produced by the app; built by pack authors, see [Sound packs](/development/addon-repos/sound-packs/) |
