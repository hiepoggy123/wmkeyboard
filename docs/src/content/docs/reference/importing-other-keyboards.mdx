...
<SettingsPath path="Layouts / Import from another keyboard" />
...
<SettingsPath path="Themes / FlorisBoard" />
...
A FlorisBoard theme that holds both a day and a night version becomes **two** themes, named for which is which, so you can put them under <SettingsPath path="Themes / Auto theme" /> as a pair.
...
- **Wallpaper-following colours.** Turn on Material You in <SettingsPath path="Themes" /> for the same effect.
...
- **Layout files are not auto-detected from a file manager.** They carry no marker that identifies them, so they go through <SettingsPath path="Layouts / Import from another keyboard" />, where you have already said what the file is.
