...
<SettingsPath>Layouts → Import from another keyboard</SettingsPath>

Pick the file and the app reads it, then asks you two things before it saves anything.
...
<SettingsPath>Themes → FlorisBoard</SettingsPath>
...
A FlorisBoard theme that holds both a day and a night version becomes **two** themes, named for which is which, so you can put them under <SettingsPath>Themes → Auto theme</SettingsPath> as a pair.
...
- **Wallpaper-following colours.** Turn on Material You in <SettingsPath>Themes</SettingsPath> for the same effect.
...
- **Layout files are not auto-detected from a file manager.** They carry no marker that identifies them, so they go through <SettingsPath>Layouts → Import from another keyboard</SettingsPath>, where you have already said what the file is.
