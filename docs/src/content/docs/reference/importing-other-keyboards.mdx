...
description: "Bring layouts, themes and dictionaries over from FlorisBoard, HeliBoard, LeanType and FUTO: what converts, what doesn't, and why."
sidebar:
  order: 6
---
...
Every AOSP-derived keyboard shares one dictionary ecosystem, and for most languages a compiled `.dict` is the only form the word list exists in. Both halves of it import here as they are: the `.combined` source lists a dictionary repository publishes, and the compiled `.dict` files HeliBoard, LeanType and FUTO download (including the ones pulled out of a GApps package).
...
Frequencies are 0 to 255 over there and 0 to 10000 here, so they are scaled rather than copied. Copying them would drop every imported word below every bundled one.

Two shapes are refused rather than half-read: a version 4 `.dict`, which is a folder of files rather than one file, and a file whose header carries a `codePointTable`, because that table changes what the rest of the file's bytes mean. Bigrams and shortcut expansions are skipped in both formats. A list here is a flat set of words, and a shortcut is an expansion rather than a word anybody typed.
...
The catch is that the result is a snapshot. A theme here stores fixed colours, so a converted theme keeps the colours your wallpaper gave it at the moment you imported it and does not follow the next wallpaper. Re-import it after a wallpaper change, or turn on Material You in <SettingsPath path="Appearance / Keyboard themes" /> to have the app's own themes follow it.
