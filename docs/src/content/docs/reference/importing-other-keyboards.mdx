...
**Keyman is a different story.** 862 Keyman key grids are already built into the app, so for most of them there's nothing to import. You can still open a `.keyman-touch-layout` file yourself, which is how you'd add a keyboard that isn't bundled or one you're writing. Full `.kmp` packages aren't read yet. See [Keyman layouts](/languages/keyman/).
