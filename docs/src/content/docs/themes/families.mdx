...
A theme can carry more than one **look**. The dark colourways are really one design in seven colours. Catppuccin is four flavours of the same palette. Solarized is a light half and a dark half, and so is Dracula, whose light half is Alucard. Rather than spilling those across the gallery as unrelated cards, they group into one **family**: one card, one preview, and a row of dots under it, one per look.
...
- **The pin** (built-ins only) is per look, not per family, because the keyboard's Appearance tool lists looks.
...
It makes a real difference to how far the built-in section scrolls. Twenty-four of the 29 built-in themes belong to a family, so grouped they fit into eleven cards.
...
Bubble, Facet, Nord, Tokyo Night and Cyberpunk stand alone, so they draw as ordinary single cards either way.
...
- **Auto theme's** light and dark slots name looks, one each or a whole set. The picker lists them flat with the family's name as a heading above its members, so you can put Solarized Light in one slot and Solarized Dark in the other. In Random mode the family's name carries a checkbox of its own that ticks every look under it, which is the quick way to say "any of the four Catppuccins". See [Theming overview](/themes/overview/#one-theme-or-a-set).
- **The Appearance tool's** pin list holds look ids, so you can pin Aurora without pinning the rest of the gradient family.
