...
- **Suggestion strip** fills the bar the word candidates sit on, and the toolbar row with it, since those two read as one bar. It sits on Auto until you pick a color, and Auto means the strip carries the board straight through, gradient, image, animation and all. Alpha works here, so a half-transparent fill tints the board rather than covering it. One thing to watch: the words on the strip take their color from the board, not from this fill, so if you paint the strip a long way from the board color, set **Suggestion text** under Panels and suggestions to match.
