...
<PhoneFrame caption="The theme gallery: the create and import actions above the cards, and an export icon on each card.">
  <Shot id="themes/gallery-export-import" alt="The theme gallery, with export and import actions above the cards." />
</PhoneFrame>
...
What a card exports depends on how the gallery is drawing it. A grouped family card exports the whole family, every look in one file. A separate card exports the single look it shows, with the other looks and the group name stripped out, so the person you send it to gets one theme rather than a family with holes in it. Flip **Theme gallery style** if you want the other one; see [Theme families and looks](/themes/families/).
...
There are three ways a theme gets imported, and they don't all behave the same way: two apply the theme immediately, one doesn't. Three more buttons on the same row, **FlorisBoard**, **HeliBoard** and **Gboard**, read someone else's format rather than this one. They're covered separately below.

### From the Import button

<SettingsPath path="Appearance / Keyboard themes" />

The **Import** button, first of the row above the cards, opens a file picker scoped to JSON-ish files. It's intentionally permissive about the exact MIME type, since file providers report a `.json` file as `text/plain` or `application/octet-stream` about as often as they get it right. Pick a `.wmtheme.json` file and it's added to Your themes under a freshly generated ID. The ID stored inside the file itself is discarded, so importing the same file twice creates two separate custom themes rather than updating one in place. Any embedded background image is written back out to local storage as part of the import.
...
:::caution
This file association is matched by filename, since a theme file carries no internal tag to identify it: a `.wmtheme.json` extension is the only signal. WM Keyboard is still offered for a file renamed to plain `.json` — it accepts JSON files by type as well as by name — but it will no longer recognize the contents as a theme, and will offer to open the file in its editor instead. The in-app Import button above isn't affected, since it doesn't check the filename.
:::

<PhoneFrame side caption="Opening a theme file from a file manager shows a one-tap import confirmation.">
...
The gallery's **FlorisBoard** button reads FlorisBoard's own `.flex` theme packages instead of `.wmtheme.json`, and converts every theme inside one into a custom theme here. A converted theme is saved without being switched on, so you can look at it before wearing it. You can open a `.flex` straight from a file manager too. A file that carries both a day and a night version becomes one theme with two looks rather than two entries. Both FlorisBoard style formats are read, the 0.4 one and the 0.5 one. What converts cleanly and what gets reported instead of guessed at are in [Importing other keyboards](/reference/importing-other-keyboards/).

### From a HeliBoard or LeanType theme

The **HeliBoard** button reads the colour themes those two keyboards share. They have no theme file at all: a theme is a line of JSON copied out of the menu at the top of their colour screen, which is why their themes circulate as forum posts. So the button asks where yours is, **Paste** or **Open a file**, and reads either. Both of that keyboard's export shapes work, the ten named colours and the forty-odd internal roles of its *all colours* mode. Like a FlorisBoard import, the theme is saved without being switched on, and the result message says how many of its colours landed. See [Importing other keyboards](/reference/importing-other-keyboards/).

### From a Gboard theme or the Rboard collection

The **Gboard** button asks where the theme is. **Open a file** reads a Gboard theme `.zip`: the file Gboard itself exports, and the format the Rboard community shares hundreds of themes in. A ZIP that holds several themes (an Rboard *pack*) lists them so you can pick one, or **Add all**. **Browse Rboard** opens a list of the packs in the Rboard repository on GitHub; the list is fetched only when you open that screen, and a pack only when you press it. The Google Play build has no Browse Rboard, so there the button opens the file picker straight away. Every theme ends on a dialog with a preview, the count of style rules that came across, and a line for each thing that could not. Like the other two conversions, a theme is saved without being switched on. Gboard draws a theme with or without key borders, and a theme that ships both becomes one theme with two looks. You can also open a Gboard theme or an Rboard pack straight from a file manager. See [Importing other keyboards](/reference/importing-other-keyboards/#gboard-themes-and-rboard-packs).

## Publishing a theme

Publishing requires no extra packaging. A repository serves the exact `.wmtheme.json` file the Export button produces. Export the theme. Drop the file into your repository's `themes/` folder. Reference it from the repository manifest. See [Addon repository format](/development/addon-repos/repo-format/) for the manifest shape and a worked theme entry.
