...
- **`assembleFullDebug`** builds every feature: ML Kit handwriting recognition, ML Kit OCR/QR/document scanning, the Harper grammar checker, the local LLM tool, and offline Whisper dictation.
- **`assembleLiteDebug`** switches all five off. That drops roughly 100 MB of ML Kit models and the Harper native library from the install, which is what makes it useful on a low-storage device.

Both flavors exist on every module so the project builds uniformly across variants. Only three modules actually carry different source code per flavor: `core/voice`, `core/intelligence` and `feature/ime` each have a real `src/full/` and `src/lite/` split. Everywhere else the flavor dimension is structural. The sources compiling under `full` and `lite` are identical. `core/config` mirrors the same five `ENABLE_*` flags as `BuildConfig` booleans so library modules can read them without depending on `:app`.
