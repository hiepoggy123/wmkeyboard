...
`unitTests` is a task in the root build, and it exists because a bare flavored task name no longer reaches the whole project. `:app` carries the `languages` flavour dimension and the library modules do not, so `:app`'s suite is `testFullIntlDebugUnitTest` while every `core/*` and `feature/*` module still declares `testFullDebugUnitTest`. Running the bare `testFullDebugUnitTest` therefore runs the libraries and silently skips all of `:app`, which is most of the tests. `unitTests` depends on both spellings. Swap in `testLiteDebugUnitTest` (or `:app:testLiteIntlDebugUnitTest`) to run a suite against the `lite` flavor. See [building from source](/development/building/) for what the flavors change.
...
<Shot id="development/diagnostics-screen" alt="Settings → About → Diagnostics: filterable app log, crash records, and a one-tap report export." />
</PhoneFrame>
