...
their original `com.wasimaster.wmkeyboard.*` layout — module boundaries follow
the packages, so a class's module is visible from its path, not its package.
The `core` engines are plain Kotlin (no Android framework types except Context
where storage demands it), which keeps the interesting logic unit-testable on
the JVM.
