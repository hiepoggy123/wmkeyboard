...
A `.wmdict` starts life as a plain-text wordlist, one entry per line, `word<space>frequency`. Lines starting with `#` are skipped, and so are blank lines. Anything else becomes an entry. A line with no space, or whose trailing token isn't a number, defaults to frequency `1` rather than being dropped. That's `DictionaryLoader`'s format, and both the bundled dictionaries and user-imported custom dictionaries parse a file exactly this way.

`DictionaryLoader` reads a second format as well, decided once on the file's first meaningful line: the AOSP `.combined` wordlist, whose `dictionary=` header is followed by indented `word=the,f=222,flags=` records. Its 0-255 frequencies are scaled onto this app's 0-10000 range by `scaleAospFrequency`, and `bigram=`/`shortcut=` records are skipped. Before that sniff existed a combined list wasn't rejected, it was *accepted wrongly*: a record line holds no space, so `word=the,f=222,flags=` became a single word at frequency 1. The compiled side of that ecosystem, a binary `.dict`, is `AospDictionary`'s job; `CustomDictionaries.import` unpacks one into this text format in place, so nothing downstream of the import needs a second reader. The downloadable catalog's own streaming parser (below) follows the same `word<space>frequency` convention, but it's a separate implementation with its own stricter filtering rather than a call into `DictionaryLoader`.
...
The generated catalog currently holds **126 entries**, down from the repo's 142 language packs, after three filters. Codes the repo spells differently are remapped (`no`→`nb`). Exact duplicates of a plainer code already present are dropped (`fil`==`tl`, `pt_br`==`pt`, `zh_cn`==`zh`), as are codes with no matching app language (`blo`, `bs`, `ccp`, `quc`, `rhg`, `zh_tw`). Finally, seven near-empty upstream stubs carrying 1–16 emoji each are dropped (`bgn`, `ceb`, `ckb`, `mni`, `su`, `syr`, `vec`).
