...
| Group | Ids | Count |
|---|---|---|
| Tools | `tool.<toolbar tool>`, for example `tool.clipboard`, `tool.gif`, `tool.sticker`, `tool.voice` | 60 |
...
3. Optionally fill `sha256` and `sizeBytes`. **Neither is required.** A manifest
   without them is valid, and its addons install normally, marked unverified. Do
