...
The schema's `type` enum does not list `vocabulary` yet, so a schema validator
...
`fileName` locates the image inside the archive, `name` is the sticker's title, and
`emojis` is its list of keywords. Despite the name, a keyword can be a word, a phrase or an
emoji. The key is still called `emojis` because every pack already out there says so. A
hand-written manifest may call the list `keywords` instead, or use both, and the two are
merged. Both fields are optional. The title and the keywords are what the sticker panel's
search matches, and they're also what [offers the sticker while typing](/emoji/stickers/#stickers-as-you-type):
the full title, or any one keyword.
...
every new tool. The other three groups are hand-maintained lists in the app.
Two of them have grown too: a new key and a new piece of toolbar chrome each
picked up a replaceable icon. The Emoji tabs group has not changed.
