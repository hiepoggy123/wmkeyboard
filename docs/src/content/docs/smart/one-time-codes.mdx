...
The chip is hidden, without the code being dropped, when the current field doesn't qualify: **Only in number fields** is on and this field isn't a number field, or you're in [incognito mode](/privacy/incognito/) (globally or for this field). Hidden rather than dropped, because you're often one tap away from the box the code belongs in.
...
| Allow notification access | — | Codes from notifications on, and access not granted yet |
