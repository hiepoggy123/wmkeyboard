...
The chip hides itself, without dropping the code, when the current field doesn't qualify. One case does that: **Only in number fields** is on and this field isn't a number field. [Incognito mode](/privacy/incognito/) doesn't hide it: reading a code out of a notification records nothing, and a login in a private tab is where the code is wanted. It hides rather than drops, because you're often one tap away from the box the code belongs in.
