...
They look similar but they're not the same feature, so the keyboard keeps them apart: separate toggles, separate limits on how many chips each may show, and separate rules about how much of the strip each one gets. Wanting saved logins in the strip says nothing about wanting the system to read your messages, and the other way round.
...
<SettingsPath path="Typing / Smart replies" />
...
- **The keyboard never sees a chip's contents.** Chips are drawn by the password manager or by the system, not by the keyboard. The keyboard only says how much space is available (a size and a chip count) and gets back an opaque view it displays as-is. Whatever username, password or suggested reply is behind a chip stays inside the other process the whole time.
- **No color or theme is applied to the chips.** The request the keyboard sends deliberately skips style customization, so the sender's own chip appearance is used rather than an approximation of the keyboard's theme. This is on purpose: a wrong guess at colors could make credential text unreadable.
...
- **Independent of the general suggestions toggle.** Turning off the main **Suggestions** toggle (word predictions above the keyboard) doesn't affect either kind of chip: they're checked separately, so you can have one without the other.
