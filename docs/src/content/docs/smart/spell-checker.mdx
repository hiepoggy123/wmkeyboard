...
2. Tap **Use Harper everywhere**, under the **System-wide** group. Android's own Spell checker screen opens. On OEM builds that hide that screen, the Language & input screen opens instead.
...
- **Runs even when WM Keyboard isn't your keyboard.** The checker lives in its own process and loads Harper (and, if enabled, your contacts) independently of the IME, so it keeps working with any other keyboard app in front. It only needs to be selected as the system spell checker. Which on-screen keyboard you're using doesn't matter.
...
- **Not fully covered by Settings search.** "English dialect," "Re-check delay," and "Use Harper everywhere" all turn up in Settings search. "Underline only, no fix popup" currently doesn't. If you're hunting for that one, go to Tools → Grammar check directly.
