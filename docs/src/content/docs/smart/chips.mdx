...
- **Detect codes, numbers and links** (default **on**), under <SettingsPath path="Tools / Clipboard" />, governs the Clipboard panel's entity strip. The fresh-copy code chip is a separate pair of switches on that same screen: **Suggest the last copy** (default **on**), since the code chip is a variant of that chip, and **Suggest copied codes in code boxes** (also default **on**), which is what allows a copied code onto the strip at all.
