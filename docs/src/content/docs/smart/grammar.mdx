...
- **A fix only touches the words it flagged.** Applying a chip (or "Fix all") replaces the flagged spans and nothing else, so the rest of what you wrote is never re-typed into the field. That's what keeps formatting alive in apps that have any: bold, colour, a checklist item in Google Keep.
