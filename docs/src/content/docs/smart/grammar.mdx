...
Each issue also lints against a specific English dialect: **American** (the default), **British**, **Canadian**, or **Australian**, chosen independently of whatever language or layout you're actually typing with, since Harper only understands English. See [Options](#options) below for where to change it.
...
- **A missing engine fails safely.** If `libharper_jni.so` somehow didn't load, the panel opens normally and just says the grammar engine isn't in this build, rather than crashing or hanging on "checking."
...
For contact-aware, dictionary-based red squiggles in every other app on your phone (not just WM Keyboard's own writing surface), see [System spell checker](/smart/spell-checker/). For the everyday typo-fixing that runs on every language and every edition, see [Autocorrect](/smart/autocorrect/).
