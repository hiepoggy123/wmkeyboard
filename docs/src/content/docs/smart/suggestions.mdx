...
<PhoneFrame caption="Word candidates in the suggestion strip, mid-word.">
  <Shot id="smart/suggestion-strip-anatomy" alt="Word candidates in the suggestion strip, mid-word." />
</PhoneFrame>
...
<PhoneFrame side caption="Word candidates followed by predicted emoji.">
  <Shot id="smart/suggestion-strip-emoji-tail" alt="Word candidates followed by predicted emoji." />
</PhoneFrame>
...
<PhoneFrame side caption="A genuinely ambiguous typo offered as two tappable candidates instead of being auto-corrected.">
  <Shot id="smart/suggestion-strip-ambiguous-correction" alt="A genuinely ambiguous typo offered as two tappable candidates instead of being auto-corrected." />
</PhoneFrame>
...
## When the cursor lands in a word
...
That chip is there for the hold, not the tap. Tapping it would replace the word with itself, so it does nothing but put the strip away. Holding it opens the [word menu](/smart/personal-dictionary/#from-the-suggestion-strip), which is how you add the word to your dictionary, open its card to respell or re-rank it, delete it, or block it, all without leaving the sentence. Holding any other chip reaches the same menu, and its **Add** and **Search all words** items are about the word the cursor is in rather than the chip you held.

**Only offer new words** (<SettingsPath path="Typing / Suggestions" />, off) takes that chip away, the same way it takes away the slot the word you are typing normally holds. With it on, every slot offers something different from what is already there, and the word menu is then only a hold on some other chip away.

A word a swipe wrote gets one thing more: the strip leads with that stroke's other readings instead of respellings of the letters standing in the field. See [Going back to a misread swipe](/typing/glide-typing/#going-back-to-a-misread-swipe).
...
<PhoneFrame caption="Punctuation chips riding after the word candidates.">
  <Shot id="smart/punctuation-chips" alt="Punctuation chips riding after the word candidates." />
</PhoneFrame>
...
<PhoneFrame caption="Suggestion chips re-cased to Title Case while Shift is armed.">
  <Shot id="smart/suggestion-strip-shift-recase" alt="Suggestion chips re-cased to Title Case while Shift is held." />
</PhoneFrame>
...
Press and hold the word on the strip. The menu that opens has **Never suggest "word"**, which puts it on the [suggestion blacklist](/smart/blacklist/), and **Delete "word"** for a word the keyboard can forget: one you learned, one still waiting to be learned, one in Android's dictionary, one you adjusted the rank of, or one from a word list you imported. **Synonyms** lists words with the same meaning, and tapping one puts it in place of the word. **Edit** opens the word card, which carries Never suggest and Delete and the rank controls besides. The [Personal dictionary](/smart/personal-dictionary/#from-the-suggestion-strip) page describes the menu and the card in full, and **Word menu** under <SettingsPath path="Typing / Suggestions" /> picks which of the optional items it shows.

The same two lists are editable from settings, if you would rather not chase the word on the strip: the [Personal dictionary](/smart/personal-dictionary/) screen for learned words, and the [suggestion blacklist](/smart/blacklist/) screen for words that should never be offered or corrected to, whether or not they were ever learned.

## Options

<SettingsPath path="Typing / Suggestions" />

<PhoneFrame side caption="The Suggestions group on the Typing settings screen.">
  <Shot id="smart/suggestions-settings-group" alt="The Suggestions group on the Typing settings screen." />
</PhoneFrame>
