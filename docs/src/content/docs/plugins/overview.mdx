...
<CardGrid>
	...
	<LinkCard
		title="Security"
		description="What the sandbox does, what it deliberately does not offer, and where its limits actually are."
...
There is no version of a plugin that gets around them — the sandbox has no
function for any of it. See [Security](/plugins/security/) for how that is
enforced and what the residual risks honestly are.
:::
...
Plugins are **off until you ask for them**. Turn them on here, then install from
the Addons screen or open a `.wmplugin` file:
