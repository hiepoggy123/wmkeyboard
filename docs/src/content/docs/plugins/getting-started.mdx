...
Open the **Plugins** tool on the keyboard, tap Cipher Tool, and you should see
your greeting.

:::tip[Keep the file manager open]
You will be replacing the ZIP and reinstalling as you go, which takes about ten
seconds each time.
:::
...
accidental infinite loop is stopped and reported rather than hanging the
keyboard. And if a plugin becomes unresponsive twice, it is switched off until
you turn it back on.
:::
...
Also worth a look: the **UI Kitchen Sink** demo — every widget on screen at
once, with a live log of the events they produce.
