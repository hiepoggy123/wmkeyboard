...
Nothing else to it. Plugins stay off until you turn them on, so start there:
...
Your code gets a few million instructions and a couple of seconds per event. An
accidental infinite loop is stopped and reported rather than left to hang the
keyboard.

A plugin that becomes unresponsive twice is switched off until you turn it back
on. You can stop that from happening: turn **Switch off a plugin that hangs**
off under <SettingsPath path="Tools / Plugins" />, where it is on by default.
The strikes are still counted and still shown, so you keep the evidence and give
up only the automatic switch-off. That is the trade you want while you are
writing plugins, or when a slow device is the real cause.
:::
