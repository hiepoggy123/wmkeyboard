...
never sees a keystroke. It sees the finished contents of its own box, nothing
else.
...
That last part is a setting: **Switch off a plugin that hangs**, on the
Tools / Plugins screen, on by default. With it off the strikes are still counted
and still shown, and only the automatic switch-off goes away.
