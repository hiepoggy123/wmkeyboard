...
| `permissions` | — | See [Permissions](/plugins/permissions/). Usually `[]`. |

:::note[Refused, not repaired]
A manifest this build does not fully understand is **refused** — including any
permission string it does not recognise. Half-understood code does not get to
run.
:::
...
reaching the network. See [Security](/plugins/security/).
