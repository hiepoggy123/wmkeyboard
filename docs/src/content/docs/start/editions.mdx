...
<PhoneFrame caption="The Tools panel on a Full build: Handwriting, Text scan, QR & barcode, Document scanner and Grammar check all appear.">
  <Shot id="start/editions-toolbox-full" alt="The Tools panel on a Full build: Handwriting, Text scan, QR & barcode, Document scanner and Grammar check all appear." />
</PhoneFrame>
...
On the Play Store build some of that library code is not in the initial install either: the LiteRT-LM runtime, ML Kit's translator, the LiteRT interpreter behind offline Whisper and the sticker editor's own background remover, and ML Kit's handwriting recogniser each arrive as an on-demand part the first time the feature is used.
...
<PhoneFrame caption="Settings → About → Version names the flavor, build type, and version code.">
  <Shot id="start/editions-about-version" alt="Settings → About → Version names the flavor, build type, and version code." />
</PhoneFrame>
