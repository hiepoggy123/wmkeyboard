...
|---|---|---|
| `arm64-v8a` | 66 MB | 8.7 MB |
| `armeabi-v7a` | 35 MB | 8.7 MB |
| `x86_64` | 76 MB | 8.7 MB |
| `universal` | 185 MB | 8.9 MB |
...
Lite leaves out the ML Kit and Harper native libraries those features need, so its APK is far smaller: 8.7 MB against 66 MB for Full on `arm64-v8a`. Exact numbers shift as dependencies update. Pick Lite if you're short on storage, or if you don't need handwriting, scanning, grammar checking, offline voice or the local LLM. It gets you everything else for a fraction of the download.
