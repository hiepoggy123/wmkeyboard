...
The bulk wipe. **Delete the learned words** takes out all five files in a single pass: the learned words, your emoji usage, the corrections you undid, your CJK picks, and language‑mix confidence. Nothing personal is left behind for the keyboard to keep guessing from. Words you added by hand go with them, because the Personal dictionary screen above writes into that same learned-words file rather than a list of its own. Snippets and imported word lists are separate files and stay put.
