...
There's no background service. The keyboard runs only while you're typing, as a normal Android input method process the system can pause or reclaim like any other app. The two heaviest things it can hold in memory are an offline Whisper voice model and a local LLM. Both are released the moment Android signals memory pressure, and both reload the next time you use them. Word lists use compact, mostly on-disk structures rather than big in-memory maps, so even large downloaded dictionaries barely touch the Java heap.
...
By default, power saving switches on whenever Android's own battery saver is on, and it's ignored while the charger is plugged in. It deliberately doesn't trigger at a fixed battery percentage, so the keyboard changing how it feels is always an answer to something you did. Gesture typing and the key‑press popup stay on even under power saving, for opposite reasons. The popup is cheap to draw. Gesture typing is the most CPU-expensive thing the keyboard does, and it stays on anyway, because dropping it would be the most noticeable loss for the people who installed the keyboard for it. Every item here is its own switch, so keep what matters to you and drop the rest.

## Where are my learned words stored, and how do I wipe them?

In a plain JSON file in the app's private storage, and nowhere else. It's `user_lexicon.json`, alongside similar files for CJK picks, emoji usage, and language‑mix confidence. Before the phone's first unlock there's no file to write to at all. Anything typed in that window stays in memory for the session and is thrown away once the device unlocks, never queued up to save later.
...
Lists every word the keyboard has learned, with a per‑word delete and a button to add your own. Words you add here show as "Added by you." Words picked up from typing show how many times they've been seen. Changes take effect immediately, even if the keyboard is already open.

<SettingsPath path="Privacy / Your data / Clear learned words" />

The bulk wipe. One button deletes the learned‑words file along with the emoji‑usage and CJK‑history files in a single pass. It doesn't touch language‑mix confidence, so that one signal survives a clear. If the keyboard's process is still running with words loaded in memory, the change won't show there until that process is torn down and rebuilt. Switching apps a few times does it, or the system reclaiming the process. Closing and reopening the app you're typing in is usually enough.
