...
In a plain JSON file in the app's private storage (`user_lexicon.json`, alongside similar files for CJK picks, emoji usage, and language‑mix confidence) — never anywhere else. Before the phone's first unlock there's no file to write to at all: anything typed during that window is held in memory for the session only and thrown away once the device unlocks, never queued up to save later.
...
<PhoneFrame caption="Personal dictionary: every learned word, with per-word delete and an Add word button.">
  {/* shot: start/faq-personal-dictionary */}
</PhoneFrame>
...
There's no in‑app feedback form — the flow is to export a diagnostic report from the app, then open an issue on the GitHub repository linked from the About screen.
