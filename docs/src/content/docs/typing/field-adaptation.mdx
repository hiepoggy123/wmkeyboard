...
<PhoneFrame caption="An email field: the bottom-row comma has become @, and the period key's long-press offers .com, .net, .org, .edu, .co.">
  {/* shot: typing/field-adaptation-email */}
</PhoneFrame>
...
<PhoneFrame caption="A URL field: the comma key has become /, with ? # & = on its long-press.">
  {/* shot: typing/field-adaptation-uri */}
</PhoneFrame>
...
<PhoneFrame caption="A phone-number field: a 4-by-4 dial pad in phone order, not calculator order.">
  {/* shot: typing/field-adaptation-numpad */}
</PhoneFrame>

The five numeric-pad field kinds each get their own 4-column keypad — digits arranged phone-pad style (1-2-3 on top, not 7-8-9 like a calculator), delete in the top-right corner, enter in the bottom-right, and the remaining two keys tailored to what that field type needs:
...
The keypad shows up whenever the field kind calls for it, with one exception: if focus has actually been rerouted to something that needs letters — an in-panel search box (emoji, dictionary, clipboard, media), a plugin's own text field, or the typing-test tool — the letter layout stays up instead, since a digits-only pad would leave those impossible to type in.
...
Don't confuse this with the standalone **Numpad** tool in the toolbox, which is a separate calculator-style keypad panel you open on purpose from any field. Its own phone-vs-calculator digit order is a toggle under that tool's own settings and has no effect on the field-adapted keypads described here — those are always phone-pad ordered.
:::
...
<PhoneFrame caption="A password field: the suggestion strip stays hidden, and pasting into it wipes the clip afterward.">
  {/* shot: typing/field-adaptation-password */}
</PhoneFrame>
...
- **Clipboard history stops recording** while a secure field has focus — new copies (and screenshot-to-clipboard captures) are skipped for as long as it's focused.
...
- **The typing-intelligence cutoff is broader than "passwords only."** Autocorrect, gesture typing, phonetic (Avro) composing, and learning all turn off for every field kind except plain, non-secure `TEXT` — that includes email and URL fields, not just passwords and numeric pads.
