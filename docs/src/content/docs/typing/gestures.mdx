...
Set a slot to **Numpad** and crossing the initial touch-slop opens the [numeric keypad panel](/tools/text-editing/#the-numeric-keypad) immediately — there's no further distance to drag, the swipe just triggers the panel and then goes inert until you release (which types nothing). It's the same panel <KeyCap>?123</KeyCap>'s long-press and the toolbar's Numpad tool open; see [Text editing](/tools/text-editing/) for what's on it.

## Deleting whole words with backspace

<SettingsPath path="Typing / Backspace" />

**Swipe to delete words** (on by default) lets a leftward drag on <KeyCap>⌫</KeyCap> take out a word at a time instead of one character per press. Backspace treats tap, hold-to-repeat, and this swipe as one gesture: a press that stays still starts the normal hold-to-repeat after the long-press delay, exactly as described on [Keys & typing basics](/typing/basics/#multi-touch-and-key-repeat); moving left past a small slop cancels that repeat and switches to word deletion instead.
...
- **Swiping across the letters** to write a whole word, or to draw handwriting instead, is controlled by one setting (what a letter-area swipe does) with two outcomes: [Glide typing](/typing/glide-typing/) for the default word-swipe mode, and [Handwriting](/tools/handwriting/#handwriting-without-opening-the-panel) for drawing ink over the keys instead.
...
Reach these from the **Typing** tile in settings, or search "Gestures", "Backspace", or "Volume keys". The Gestures group's language-switching settings (Arrows on spacebar, Spacebar shows, Spacebar text) are covered on [Switching languages](/languages/switching/#swiping-the-spacebar) instead of here. For the exhaustive, field-by-field breakdown see [Typing settings](/reference/settings/typing/).

## Details & edge cases

- **Numpad is a discrete action, not a distance-based one.** Unlike Cursor, which keeps stepping for as long as you keep dragging, a swipe assigned to Numpad only needs to clear the initial slop once — the panel opens immediately and the rest of the drag does nothing.
...
See [Keys & typing basics](/typing/basics/) for tap and long-press behavior on individual keys, [Switching languages](/languages/switching/) for the spacebar's Language swipe and the hold-drag picker, [Glide typing](/typing/glide-typing/) for swiping across the letters, and [Text editing](/tools/text-editing/) for the numpad panel and other cursor tools.
