...
- **Erase** and **Restore** — a round brush with a size slider. Erase clears pixels to transparent, Restore paints them back. **Undo** goes back one stroke at a time, twelve deep.
...
Between sessions it isn't. When you add a sticker from a photo, the keyboard keeps that photo (resized to 1536px on its long edge, as WebP) alongside your packs, so **Edit the image** reopens the picture you started from rather than the flattened cut-out. That's what makes a second pass practical: you can re-crop, redo the background, or change your mind about the border right after saving, without the erased parts being gone for good. Stickers that arrived some other way (saved from Klipy or GIPHY, imported in a pack) have no separate original, so they reopen as themselves; the first time you edit one, the version you're replacing is kept as its original.

Those kept photos never leave the phone. Exporting a `.wmstickers` pack writes the manifest and the finished stickers only, the settings backup's sticker section walks the same list, and Android's own cloud backup and device transfer skip the folder entirely. They do take up space, so <SettingsPath path="About / Storage / Sticker packs" /> lists them as their own row ("Images you made stickers from") and you can delete just that row to get the space back, keeping every sticker.
