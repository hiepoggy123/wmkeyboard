...
- Pick exactly **one** still image and it opens in the [sticker editor](#the-sticker-editor) first, instead of being added as-is. Pick several and they're all added the old way; an editor thirty times over isn't a flow anyone wants.
...
Edits replace the image. There's no stored original, so re-editing a sticker starts from the edited version, and cropping again after you've erased something warns you first, because a new crop throws the erasing away. Nothing is written until you press Save.
