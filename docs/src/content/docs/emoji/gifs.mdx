...
<PhoneFrame caption="The GIF panel, showing trending results on open.">
  <Shot id="emoji/gif-panel-trending" alt="The GIF panel, showing trending results on open." />
</PhoneFrame>
...
<PhoneFrame side caption="A field that can't accept images. The panel dims and shows the standing notice.">
  <Shot id="emoji/gif-no-rich-media" alt="A field that can't accept images — the panel dims and shows the standing notice." />
</PhoneFrame>
...
<PhoneFrame caption="Long-press menu over a GIF result: Copy and Report.">
  <Shot id="emoji/gif-long-press" alt="Long-press menu over a GIF result: Copy and Report." />
</PhoneFrame>
...
You only need one working key, Klipy or GIPHY, for the tool to work, except on the F-Droid build, which already has Commons. Configuring both Klipy and GIPHY adds a second source to search. **Send GIFs as** is only shown here, and the sticker tool's **Send stickers as** only on its own page. Each governs the panel it sits under, so neither page offers the other's setting. See [Stickers](/emoji/stickers/) for that one.
