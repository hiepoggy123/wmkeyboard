...
**A handful of letters fall back to an older block.** Beyond Double-struck's gaps, non-bold Fraktur is missing astral forms for C, H, I, R, and Z, so it falls back to the same Letterlike Symbols block. Non-bold Script is missing several letters in both cases. Bold Fraktur and Bold Script have no such gaps, since their Unicode blocks are complete.
