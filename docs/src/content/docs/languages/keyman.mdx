...
<SettingsPath path="Settings > Languages > (a language) > Typing rules" />

The row sits under the layout it belongs to and tells you which state you're in. Press it to download; press it again later to remove them. Nothing is fetched until you press it, and the row names where it's fetching from.
...
This is for keyboards that aren't bundled, and for one you're writing yourself. A touch layout file holds no rules, so an imported layout types its key caps and has no Typing rules row. Full `.kmp` packages aren't supported yet.
