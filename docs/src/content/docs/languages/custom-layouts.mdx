...
There's no default value to quote for "which layouts exist" — a fresh install starts with no custom layouts of your own. The active layout defaults to QWERTY, and the initial enabled set is QWERTY plus WM Keyboard's three Bengali built-ins (Avro, Probhat, and Jatiya) — see [Bengali](/languages/bengali/) for why.
